...
def build_month_context(year, month):
    ...
    month_name = get_month_name(month_num, True)
    ...
    for day in range(1, days_in_month + 1):
        ...
        weeks_map.setdefault(wn, []).append(day)
    weeks = []
    for wn in sorted(weeks_map):
        workdays = weeks_map[wn]
        start, end = workdays[0], workdays[-1]
        label = (
            f"{month_name} {start}" if start == end else f"{month_name} {start}-{end}"
        )
...
def fetch_sheet(month, year, sheet_id=None, dest=None):
    ...
    dest = (
        Path(dest)
        if dest
        else Path(tempfile.gettempdir()) / f"weekly-hours-sheet-{month}-{year}.xlsx"
    )
...
def get_sheet_names(month, year):
    aliases = ["Sep", "Sept"] if month == "Sep" else [month]
    return [f"{m} {year}" for m in aliases]
...
def read_pto(month, year, pto_path=None):
    if not pto_path:
        raise ValueError(
            "pto_path (local xlsx fetched from the Google Sheet) is required"
        )
    p = Path(pto_path)
    if not p.exists():
    ...
    for col_idx in range(FIRST_DAY_COL, backup_col):
        ...
        try:
            ...
            continue
        if 1 <= day_num <= 31:
            day_cols.append((col_idx, day_num, is_purple(cell)))
    result = {}
    for row_idx in range(DATA_START_ROW, ws.max_row + 1):
        employee_name = str(ws.cell(row_idx, COLLAB_COL).value or "").strip()
        if not employee_name:
            continue
        norm = normalize_value(employee_name)
        absences = []
        ...
        backups_text = extract_cell_text(ws.cell(row_idx, backup_col))
        notes_text = extract_cell_text(ws.cell(row_idx, notes_col))
        result[norm] = {
            "employeeName": employee_name,
            "absences": absences,
            "backupsText": backups_text,
...
def read_team(team_path=None):
    if not team_path:
        raise ValueError(
            "team_path (local xlsx fetched from the Google Sheet) is required"
        )
    p = Path(team_path)
    if not p.exists():
...
# Collaborators in PTO but intentionally excluded from team allocation — skip without warning.
...
def build_comments(weeks):
    ...
    for typ, days in mp.items():
        ...
        rng_txt = ",".join(
            f"{r['start']}" if r["start"] == r["end"] else f"{r['start']}-{r['end']}"
            for r in ranges
        )
...
def build_report(pto_data, team_data, month, year):
    ...
    for norm_name, data in pto_data.items():
        ...
        absence_days_set = {
            a["dayNumber"]
            for a in data["absences"]
            if normalize_value(a["type"]) in COVERABLE_TYPES
        }
        ...
        for br in backup_ranges:
            for day in range(br["startDay"], br["endDay"] + 1):
                ...
                if day not in absence_days_set:
                    print(
                        f"WARN {emp['short_name']}: backup {br['originalBackupName']} on day {day} without PTO/ML/O, skipping that day"
                    )
        bc = {}
        for br in backup_ranges:
            ...
            for day in range(br["startDay"], br["endDay"] + 1):
                if day not in absence_days_set:
                    continue  # only cover PTO/ML/O days (excludes weekends & holidays)
                try:
                    d = date(year, month_num, day)
                except:
                    continue
                if d.weekday() >= 5:
                    continue
                wn = get_week_of_month(d)
                bc.setdefault(br["backupName"], {}).setdefault(
                    wn,
                    {
                        "backupName": be["short_name"],
                        "totalHours": 0,
                        "workdays": set(),
                        "employeeData": be,
                    },
        ...
        for bn, wm in bc.items():
            for wn, cd in wm.items():
                if wn not in weekly:
                    continue
                w = weekly[wn]
                wds = sorted(cd["workdays"])
                w["backups"].append(
                    {
                        "name": cd["employeeData"]["short_name"],
                        "role": cd["employeeData"]["job_role"],
                        "wave": cd["employeeData"]["wave"],
                        "product": cd["employeeData"]["team"],
                        "pilar": cd["employeeData"]["module"],
                        "startDay": min(wds),
                        "endDay": max(wds),
                        "hours": cd["totalHours"],
                        "workdays": wds,
        ...
        for w in all_weeks:
            ...
            for d in wds:
                dn = d.day
                raw = absence_map.get(dn)
                if not raw:
                    if dn in bench_days:
                        days.append(
                            {
                                "dayNumber": dn,
                                "dayName": get_day_name(d),
                                "status": "Bench",
                                "hours": 0,
                            }
                        )
            ...
            meta = next(
                (x for x in ctx["weeks"] if x["weekNumber"] == w["weekNumber"]), None
            )
            week_details.append(
                {
                    "weekNumber": w["weekNumber"],
                    "label": meta["label"] if meta else f"Week {w['weekNumber']}",
                    "totalHours": w["totalWorkedHours"],
                    "days": days,
                }
            )
        ...
        comments = "\n".join(
            filter(None, [build_team_note(team_changes), build_comments(all_weeks)])
        )
        employee = {
        ...
        if target is not None:
                ...
                for b in w["backups"]:
                    ...
                    if key not in backup_map:
                        backup_map[key] = {
                            ...
                            "product": b["product"],
                            "pilar": b["pilar"],
                            "weeklyHours": [0] * ctx["weekCount"],
                            "totalHours": 0,
                            "allWorkdays": [],
                            "coveredDaysByWeek": {},
                        }
                    bm = backup_map[key]
                    bm["weeklyHours"][wi] += b["hours"]
                    bm["totalHours"] += b["hours"]
            ...
            for bm in backup_map.values():
                ...
                for wi in range(ctx["weekCount"]):
                    wds_w = (
                        [
                            d
                            for d in workdays
                            if get_week_of_month(d) == all_weeks[wi]["weekNumber"]
                        ]
                        if wi < len(all_weeks)
                        else []
                    )
                    ...
                    wds.append(
                        {
                            "weekNumber": (
                                all_weeks[wi]["weekNumber"]
                                if wi < len(all_weeks)
                                else wi + 1
                            ),
                            "label": (
                                week_details[wi]["label"]
                                if wi < len(week_details)
                                else f"Week {wi+1}"
                            ),
                            ...
                            "days": [
                                {
                                    ...
                                    "dayName": get_day_name(d),
                                    "status": "",
                                    "hours": 8 if d.day in covered else 0,
                                }
                                for d in wds_w
                            ],
                        }
                ...
                rng_txt = ",".join(
                    (
                        f"{r['start']}-{r['end']}"
                        if r["end"] - r["start"] + 1 >= 4
                        else ",".join(str(x) for x in range(r["start"], r["end"] + 1))
                    )
                    for r in ranges
                )
...
def section_color(col, week_count, weeks_detail_cols):
    ...
    gold_end = (
        5 + weeks_detail_cols
    )  # weeks area = Mon-Fri days + weekly Hrs (no per-week label col)
    if col <= gold_end:
        return GOLD
    if col <= gold_end + 2:
        return GREEN
...
def write_weekly_sheet(wb, data, title="Weekly Hours"):
    ...
    Each week is represented by a single 'Hrs' column (no individual day columns).
    """
    employees = []
    ...
    # Each week block = just one Hrs column (no individual day columns).
    weeks_detail_cols = week_count
    ...
    for _ in range(week_count):
        widths.append(10)  # one Hrs column per week
    ...
    # Divider lines: after identity (col 5), after all weeks, after Work Hrs, after Comments, outer right.
    divider_cols = {5, 5 + weeks_detail_cols, abs_col + 1, comments_col, total_cols}
    ...
    # Row 1 — "Week N" headers (one cell per week, no merge needed)
    for c in range(1, total_cols + 1):
        cell = ws.cell(1, c)
        sc = section_color(c, week_count, weeks_detail_cols)
        cell.fill = fill(sc)
    ...
    for i, h in enumerate(headers, 1):
        cell = ws.cell(2, i, h)
        sc = section_color(i, week_count, weeks_detail_cols)
        cell.fill = fill(sc)
        cell.font = header_font
        cell.alignment = center
        cell.border = make_border(i, bottom_dark=True)

    work_refs = {}  # id(emp) -> "'Weekly Hours'!<col><row>" of its Work Hrs cell
    tag_refs = {}  # id(emp) -> "'Weekly Hours'!<col><row>" of its Tag cell
    ...
    for emp in employees:
        ...
        is_emergency = (
            not is_backup and emp.get("absHrs", 0) != -1 and emp["absHrs"] > 80
        )
        ws.cell(row_idx, 1, emp["fullName"])
        ws.cell(row_idx, 2, emp["role"])
        ws.cell(row_idx, 3, emp["wave"])
        ws.cell(row_idx, 4, emp["product"])
        ws.cell(row_idx, 5, emp["pilar"])
        # Write one Hrs value per week (no day columns)
        hrs_cells = []
        ...
        # Work Hrs = SUM of the weekly Hrs cells
    ...
    ws.cell(row_idx, abs_col).value = (
        f"=SUM({abs_letter}{data_start}:{abs_letter}{data_end})"
    )
    ws.cell(row_idx, abs_col).font = Font(bold=True)
    ws.cell(row_idx, abs_col).alignment = center
    work_letter = get_column_letter(abs_col + 1)
    ws.cell(row_idx, abs_col + 1).value = (
        f"=SUM({work_letter}{data_start}:{work_letter}{data_end})"
    )
    ws.cell(row_idx, abs_col + 1).font = Font(bold=True)
    ws.cell(row_idx, abs_col + 1).alignment = center
    total_backup = sum(
